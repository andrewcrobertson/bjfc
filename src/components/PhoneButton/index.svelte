...
<div class="flex">
  {#if phone === null}
    <div class="{rootClass} cursor-default inline-flex flex-grow items-center leading-none text-sm bg-gray-100 border border-gray-300 px-3 py-3 ">
      <PhoneIcon class="inline w-5 h-5 text-{colour}-500 mr-2" />
      Not Listed
    </div>
    <a
      ...
      <ChatAltIcon class="inline w-5 h-5 text-{colour}-500" />
    </a>
  {:else}
    <a class="{rootClass} inline-flex flex-grow items-center leading-none text-sm border border-gray-300 px-3 py-3 " href="tel:{sanitisePhone(phone)}">
      <PhoneIcon class="inline w-5 h-5 text-{colour}-500 mr-2" />
      {phone}
    </a>
